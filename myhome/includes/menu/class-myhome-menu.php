	...
	class My_Home_Menu {
		...
		public function __construct() {
			...
			add_action( 'init', function () {
				$file = WP_PLUGIN_DIR . '/mega_main_menu/src/css/cache.skin.css';

				if ( ! file_exists( $file ) || filesize( $file ) <= 100000 ) {
					$this->update_mmm_config();
				}
			} );
		}
		...
		public function set_logo( $nav_menu ) {
			if (
				! class_exists( 'mega_main_init' )
				|| ! has_nav_menu( 'mh-primary' )
			...
			$logo     = ob_get_clean();
			$html     = new simple_html_dom( $nav_menu );
		...
		public function add_elements( $items, $args ) {
			...
			if ( ! empty( $panel ) && $panel ) {
				...
				$panel_link = apply_filters( 'myhome_panel_link', $panel_link );
                ...
                <span class="item_link">
                    <span class="link_content">
                        <a
                                ...
                                title="<?php esc_attr_e( 'Submit property', 'myhome' ); ?>"
                        >
